...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // reject on timeout
            if (elapsedTime >= (5 * 60)) {
                reject('Signature is older than 5 minutes.');
            }
        ...
        return new Promise(async (resolve, reject) => {
            for (let block of self.chain) {
                if (await block.validate()) {
                    if (block.height > 0) {
                        let previousBlock = self.chain.filter(validatedBlock => validatedBlock.height === block.height - 1)[0];
                        if (block.previousBlockHash !== previousBlock.hash) {
                            errorLog.push(`Blockchain catastrophe. Blocks are invalid, height ${block.height} and ${block.height - 1}`);
                        }
                    }
                } else {
